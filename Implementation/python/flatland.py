...
def my_flatlandSpaceStations(n, c):
    c.sort()
    q = []
    
    #head
    heapq.heappush(q, (-(c[0] - 0), 0))
    
    for i in range(1, len(c)):
        if (c[i] + c[i-1]) % 2 == 0:
            center = (c[i] + c[i-1]) // 2
            heapq.heappush(q, (-((c[i] - c[i-1]) // 2), center))            
        else:
            center_l = (c[i] + c[i-1]) // 2
            center_r = center_l + 1
            
            heapq.heappush(q, (-((c[i] - c[i-1]) // 2), center_l))
            heapq.heappush(q, (-((c[i] - c[i-1]) // 2), center_r))                
    
    #tail
    heapq.heappush(q, (-(n - 1 - c[len(c) - 1]), n - 1))        
...
def flatlandSpaceStations(n, c):
    ...
    for i in range(1, len(c)):
        max_dist = max((c[i] - c[i-1]) // 2, max_dist)
        
    max_dist = max(n - 1 - c[len(c) - 1], max_dist)        
    return max_dist    
